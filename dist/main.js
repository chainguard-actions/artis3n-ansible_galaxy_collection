...
try {
    ...
    const collectionLocation = getInput('collection_dir');
    const willBuild = (getInput('build').toLowerCase().trim() || 'true') == 'true';
    const willPublish = (getInput('publish').toLowerCase().trim() || 'true') == 'true';
...
function prepareConfig(collectionLocation) {
    ...
    try {
        const configContent = yamlLoad(readFileSync(galaxyConfigFilePath, 'utf8'));
